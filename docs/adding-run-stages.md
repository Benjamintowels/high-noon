...
2. **Create a stage config**:
   ...
   - **Tier profile:** `tier_profile = &"dry_gulch"` = bandits only — regulars stay **unarmed melee** (tier only scales toughness); elites carry shotgun then Winchester (~4m). First encounter area hybrid drip: first **5** unarmed, then revolvers for the rest of that area's budget; later areas stay unarmed. In runs, only elites drop weapons. No gun-armor / bullet reflect (`disable_enemy_gun_armor = true`). Other zones use `&"default"` (full ladder incl. armored).
...
3. **Create the zone scene** (copy `zone_1.tscn`):
   ...
   - **Encounter areas (preferred):** `RunEncounterAreas` with `Marker3D` children using [`run_encounter_area.gd`](../gameplay/runs/run_encounter_area.gd). Set `area_id` to match a `wave_groups` id, add a child `Trigger` `Area3D` + `CollisionShape3D` (preferred; resize/move in-editor — falls back to `trigger_radius` if missing), child spawn Marker3Ds (**one immediate mini-elite per marker** — red brawl aura + 2× HP), optional `DripSpawn` Marker3D to hand-place budget drip spawns, a child `Chest` Marker3D for the kill-gated loot chest, and a child `GateWall` ([`encounter_gate_wall.tscn`](../gameplay/runs/encounter_gate_wall.tscn)) that blocks the path until the pocket is cleared (then sinks into the ground). Place `Trigger` volumes on the approach / far side of each GateWall so players cannot walk past without firing the pocket. Move the roots in-editor to place fights. Group: `run_encounter_area`.
   - **Spawn marker naming (editor):** name each spawn Marker3D `{Enemy}{Weapon}{optionalNumber}` — parser in [`run_encounter_spawn_spec.gd`](../gameplay/runs/run_encounter_spawn_spec.gd). Examples: `BanditRevolver1`, `RedoLightsaber`, `TownspersonUnarmed`, `SheriffShotgun2`. Enemy tokens: `Bandit`, `Redo`, `Townsperson`, `Townsfolk`, `Sheriff`, `Pavel`, `Undead`. Weapon tokens match `GroyperWeapons.Id` without underscores (`Unarmed`, `Revolver`, `Sword2H`, `Lightsaber`, …). Legacy `SpawnA` / `SpawnB` still work (Bandit + tier default). Reserved child names: `Trigger`, `Chest`, `DripSpawn`, `GateWall*`.
