...
static func register_standup_library(
	...
	var stand_path := StringName("%s/%s" % [library_name, clip_name])
	if animation_player == null:
		return StringName()
	if animation_player.has_animation(stand_path):
		return stand_path

	var raw := RigAnimUtils.load_skeleton_animation(merged_scene, meshy_clip)
	if raw == null:
		push_warning("MeshyLassoStandup: failed to load stand up clip '%s'." % meshy_clip)
		return StringName()
	...
	return stand_path if animation_player.has_animation(stand_path) else StringName()
...
static func attach_standup_branch(
	...
	var stand_path := register_standup_library(
		animation_player,
		library_name,
		clip_name,
		merged_scene,
...
static func get_stand_duration(animation_player: AnimationPlayer, stand_path: StringName) -> float:
	if animation_player == null or stand_path.is_empty():
		return 1.0
	var anim := animation_player.get_animation(stand_path)
	if anim == null:
		return 1.0
...
static func update_smoothed_blend(current: float, progress: float, delta: float) -> float:
