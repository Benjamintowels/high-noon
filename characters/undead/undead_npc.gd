extends UndeadActor
class_name UndeadNpc

const UndeadAnimConfigScript := preload("res://characters/undead/undead_anim_config.gd")
const UndeadAnimUtilsScript := preload("res://characters/undead/undead_anim_utils.gd")
const UndeadMeleeStrikeScript := preload("res://characters/undead/undead_melee_strike.gd")
const FactionIdsScript := preload("res://gameplay/faction/faction_ids.gd")
const DuelHitTestScript := preload("res://gameplay/duel/duel_hit_test.gd")
const BloodSplatterFXScript := preload("res://gameplay/fx/blood_splatter_fx.gd")
const GameAudioScript := preload("res://gameplay/audio/game_audio.gd")

enum AiState {
	PATROL_IDLE,
	PATROL_WALK,
	CHASE,
	COMBAT_DECIDING,
	BLOCKING,
	ATTACK_WINDUP,
	ATTACKING,
	ROLLING,
	RELOCATING,
	PARRY_STUNNED,
	SPRINT_APPROACH,
}

enum AttackKind {
	SWORD_SLASH,
	CHARGED_UPWARD,
	SPRINT_SPIN,
}

const GRAVITY := 22.0
const WALK_SPEED := 2.1
const RUN_SPEED := 4.8
const BLEND_SPEED := 8.0
const LOCOMOTION_STOP_SPEED := 0.08
const LOCOMOTION_RUN_SPEED := 3.2
const DETECT_RANGE := 14.0
const ATTACK_RANGE := UndeadMeleeStrikeScript.RANGE
const ATTACK_PLAYBACK_SPEED := 2.0
const SPRINT_APPROACH_MIN := 4.5
const SPRINT_APPROACH_MAX := 11.0
const SPRINT_ATTACK_CHANCE := 0.34
const CHARGED_ATTACK_CHANCE := 0.38
const ROLL_AWAY_CHANCE := 0.06
const DECISION_MIN := 0.45
const DECISION_MAX := 1.1
const MELEE_DECISION_MIN := 0.08
const MELEE_DECISION_MAX := 0.2
const MELEE_ATTACK_CHANCE := 0.74
const MELEE_BLOCK_CHANCE := 0.08
const BLOCK_DURATION_MIN := 1.2
const BLOCK_DURATION_MAX := 2.4
const CHASE_BLOCK_CHANCE := 0.05
const PARRY_FOLLOWUP_ATTACK_CHANCE := 0.78
const PARRY_FOLLOWUP_REBLOCK_CHANCE := 0.30
const PARRY_REBLOCK_HOLD_MIN := 1.0
const PARRY_REBLOCK_HOLD_MAX := 2.0
const PATROL_IDLE_MIN := 2.0
const PATROL_IDLE_MAX := 5.0
const PATROL_WALK_MIN := 2.0
const PATROL_WALK_MAX := 4.5
const ROAM_RADIUS := 5.5
const MAX_HEALTH := BulletHitDamageScript.DEFAULT_MAX_HEALTH
const ROLL_SPEED := 6.2
const RELOCATE_ARRIVE_DIST := 0.85
const GUN_AIM_ROLL_DELAY_MIN := 0.2
const GUN_AIM_ROLL_DELAY_MAX := 1.5
const GUN_AIM_ROLL_COOLDOWN := 2.5
# Hostile/player group scans think on these intervals (staggered per NPC)
# instead of every physics tick.
const HOSTILE_SCAN_INTERVAL := 0.3
const GUN_AIM_SCAN_INTERVAL := 0.2

@export var sight_range := DETECT_RANGE
@export var roam_radius := ROAM_RADIUS
@export var prefer_last_attack_target := false
@export_group("Sword Block Animations")
@export var parry_hold_meshy_clip: StringName = UndeadAnimConfigScript.MESHY_SWORD_PARRY
@export var parry_clash_meshy_clip: StringName = UndeadAnimConfigScript.MESHY_SWORD_PARRY_BACKWARD

@export_group("Animation Preview")
@export var editor_preview_animation := &"idle":
	set(value):
		editor_preview_animation = value
		if Engine.is_editor_hint():
			call_deferred("_play_editor_preview")

var _ai_state := AiState.PATROL_IDLE
var _state_timer := 0.0
var _decision_timer := 0.0
var _windup_timer := 0.0
var _combat_target: Node3D
var _last_attack_target: Node3D
var _hostile_scan_accum := randf_range(0.0, HOSTILE_SCAN_INTERVAL)
var _gun_aim_scan_accum := randf_range(0.0, GUN_AIM_SCAN_INTERVAL)
var _combat_idle_blend := 0.0
var _walk_direction := Vector3.ZERO
var _locomotion_blend := 0.0
var _roam_center := Vector3.ZERO
var _attack_kind := AttackKind.SWORD_SLASH
var _attack_elapsed := 0.0
var _attack_timer := 0.0
var _attack_struck := false
var _attack_cooldown := 0.0
var _roll_direction := Vector3.ZERO
var _roll_timer := 0.0
var _roll_duration := 0.0
var _relocate_target := Vector3.ZERO
var _gun_aim_roll_pending := false
var _gun_aim_roll_committed := false
var _gun_aim_roll_timer := 0.0
var _gun_aim_roll_threat: Node3D
var _gun_aim_roll_cooldown := 0.0


func _on_actor_ready() -> void:
	add_to_group("undead_npc")
	add_to_group("ruins_enemy")
	add_to_group("cave_enemy")
	add_to_group("duel_target")
	_bind_hitbox_nodes()
	_setup_animation_library()
	_setup_animation_tree()
	_setup_combat_navigation()
	_setup_combat_ragdoll()
	setup_npc_locomotion_audio()
	call_deferred("_finalize_spawn")


func _finalize_spawn() -> void:
	refresh_patrol_anchor()


func refresh_patrol_anchor() -> void:
	snap_to_floor()
	_roam_center = global_position
	_prime_idle_pose()
	_sync_hitbox_debug_visibility()
	_begin_patrol_idle()


func _physics_process(delta: float) -> void:
	if Engine.is_editor_hint():
		return
	if _defeated:
		return

	tick_melee_stun(delta)
	_attack_cooldown = maxf(_attack_cooldown - delta, 0.0)
	_gun_aim_roll_cooldown = maxf(_gun_aim_roll_cooldown - delta, 0.0)

	if not is_on_floor():
		velocity.y -= GRAVITY * delta
	else:
		velocity.y = minf(velocity.y, 0.0)

	_update_combat_target(delta)
	_update_player_gun_aim_threat(delta)
	_try_execute_committed_gun_aim_roll()
	match _ai_state:
		AiState.PATROL_IDLE:
			_process_patrol_idle(delta)
		AiState.PATROL_WALK:
			_process_patrol_walk(delta)
		AiState.CHASE:
			_process_chase(delta)
		AiState.COMBAT_DECIDING:
			_process_combat_deciding(delta)
		AiState.BLOCKING:
			_process_blocking(delta)
		AiState.ATTACK_WINDUP:
			_process_attack_windup(delta)
		AiState.ATTACKING:
			_process_attacking(delta)
		AiState.ROLLING:
			_process_rolling(delta)
		AiState.RELOCATING:
			_process_relocating(delta)
		AiState.PARRY_STUNNED:
			_process_parry_stunned(delta)
		AiState.SPRINT_APPROACH:
			_process_sprint_approach(delta)

	move_and_slide()
	var horizontal_speed := Vector2(velocity.x, velocity.z).length()
	_update_combat_idle_blend(delta)
	if _ai_state not in [
		AiState.ATTACKING,
		AiState.BLOCKING,
		AiState.PARRY_STUNNED,
		AiState.ATTACK_WINDUP,
	]:
		_update_locomotion_blend(delta, horizontal_speed)
	update_npc_locomotion_audio(
		delta,
		horizontal_speed,
		horizontal_speed > LOCOMOTION_STOP_SPEED,
		horizontal_speed >= LOCOMOTION_RUN_SPEED
	)


func get_faction_id() -> StringName:
	return FactionIdsScript.RUINS


func _get_max_health() -> int:
	return MAX_HEALTH


func is_blocking() -> bool:
	return _ai_state == AiState.BLOCKING


func _can_block_melee(hit_info: Dictionary) -> bool:
	return (
		_blocking
		and _ai_state == AiState.BLOCKING
		and bool(hit_info.get("melee", false))
		and _is_facing_attack(hit_info)
	)


func apply_melee_stun(duration: float) -> void:
	if _ai_state == AiState.PARRY_STUNNED:
		return
	_melee_stun_timer = maxf(_melee_stun_timer, duration)


func _setup_animation_library() -> void:
	if _animation_player == null:
		push_error("UndeadNpc: missing AnimationPlayer.")
		return
	var library := load(UndeadAnimConfigScript.LIB_PATH) as AnimationLibrary
	if library == null:
		push_error("UndeadNpc: failed to load animation library.")
		return
	if _animation_player.has_animation_library(UndeadAnimConfigScript.LIBRARY):
		_animation_player.remove_animation_library(UndeadAnimConfigScript.LIBRARY)
	_animation_player.add_animation_library(UndeadAnimConfigScript.LIBRARY, library)
	_apply_block_clip_overrides()


func _apply_block_clip_overrides() -> void:
	if (
		parry_hold_meshy_clip == UndeadAnimConfigScript.MESHY_SWORD_PARRY
		and parry_clash_meshy_clip == UndeadAnimConfigScript.MESHY_SWORD_PARRY_BACKWARD
	):
		return

	var library := _animation_player.get_animation_library(UndeadAnimConfigScript.LIBRARY)
	if library == null:
		return

	var hold_pose := UndeadAnimUtilsScript.bake_parry_pose(UndeadAnimConfigScript.PARRY_SCENE)
	if hold_pose != null:
		library.add_animation(UndeadAnimConfigScript.CLIP_PARRY_POSE, hold_pose)

	var clash := UndeadAnimUtilsScript.load_clip(
		UndeadAnimConfigScript.PARRY_BACKWARD_SCENE,
		Animation.LOOP_NONE
	)
	if clash != null:
		clash.resource_name = "parry_backward"
		library.add_animation(UndeadAnimConfigScript.CLIP_PARRY_BACKWARD, clash)


func _setup_animation_tree() -> void:
	if _animation_tree == null or _animation_player == null:
		return

	var idle_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_IDLE)
	var combat_idle_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_COMBAT_IDLE)
	var walk_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_WALK)
	var run_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_RUN)
	var sword_slash_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_SWORD_SLASH)
	var charged_upward_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_CHARGED_UPWARD)
	var sprint_spin_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_SPRINT_SPIN)
	var parry_pose_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_PARRY_POSE)
	var parry_backward_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_PARRY_BACKWARD)
	var roll_path := UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_ROLL_DODGE)

	for clip_path: StringName in [
		idle_path, combat_idle_path, walk_path, run_path, sword_slash_path,
		charged_upward_path, sprint_spin_path, parry_pose_path, parry_backward_path, roll_path
	]:
		if not _animation_player.has_animation(clip_path):
			push_error("UndeadNpc: missing clip %s" % clip_path)
			return

	var idle_node := AnimationNodeAnimation.new()
	idle_node.animation = idle_path
	var combat_idle_node := AnimationNodeAnimation.new()
	combat_idle_node.animation = combat_idle_path
	var combat_idle_blend := AnimationNodeBlend2.new()
	var idle_subtree := AnimationNodeBlendTree.new()
	idle_subtree.add_node(UndeadAnimConfigScript.COMBAT_IDLE_BLEND, combat_idle_blend)
	idle_subtree.add_node(&"PatrolIdleAnim", idle_node)
	idle_subtree.add_node(&"CombatIdleAnim", combat_idle_node)
	idle_subtree.connect_node(&"output", 0, UndeadAnimConfigScript.COMBAT_IDLE_BLEND)
	idle_subtree.connect_node(UndeadAnimConfigScript.COMBAT_IDLE_BLEND, 0, &"PatrolIdleAnim")
	idle_subtree.connect_node(UndeadAnimConfigScript.COMBAT_IDLE_BLEND, 1, &"CombatIdleAnim")
	var walk_node := AnimationNodeAnimation.new()
	walk_node.animation = walk_path
	var run_node := AnimationNodeAnimation.new()
	run_node.animation = run_path

	var locomotion_blend := AnimationNodeBlendSpace1D.new()
	locomotion_blend.add_blend_point(idle_subtree, 0.0)
	locomotion_blend.add_blend_point(walk_node, 0.5)
	locomotion_blend.add_blend_point(run_node, 1.0)
	locomotion_blend.min_space = 0.0
	locomotion_blend.max_space = 1.0

	var parry_pose_node := AnimationNodeAnimation.new()
	parry_pose_node.animation = parry_pose_path
	var block_blend := AnimationNodeBlend2.new()
	UndeadAnimUtilsScript.configure_block_pose_blend(block_blend)

	var sword_slash_node := AnimationNodeAnimation.new()
	sword_slash_node.animation = sword_slash_path
	var sword_slash_time_scale := AnimationNodeTimeScale.new()
	var sword_slash_shot := AnimationNodeOneShot.new()
	CombatAnimTransitionsScript.configure_one_shot(
		sword_slash_shot,
		CombatAnimTransitionsScript.ATTACK_FADEIN,
		CombatAnimTransitionsScript.ATTACK_FADEOUT
	)

	var charged_upward_node := AnimationNodeAnimation.new()
	charged_upward_node.animation = charged_upward_path
	var charged_upward_time_scale := AnimationNodeTimeScale.new()
	var charged_upward_shot := AnimationNodeOneShot.new()
	CombatAnimTransitionsScript.configure_one_shot(
		charged_upward_shot,
		CombatAnimTransitionsScript.ATTACK_FADEIN,
		CombatAnimTransitionsScript.ATTACK_FADEOUT
	)

	var sprint_spin_node := AnimationNodeAnimation.new()
	sprint_spin_node.animation = sprint_spin_path
	var sprint_spin_time_scale := AnimationNodeTimeScale.new()
	var sprint_spin_shot := AnimationNodeOneShot.new()
	CombatAnimTransitionsScript.configure_one_shot(
		sprint_spin_shot,
		CombatAnimTransitionsScript.ATTACK_FADEIN,
		CombatAnimTransitionsScript.ATTACK_FADEOUT
	)

	var parry_backward_node := AnimationNodeAnimation.new()
	parry_backward_node.animation = parry_backward_path
	var parry_stun_shot := AnimationNodeOneShot.new()
	CombatAnimTransitionsScript.configure_one_shot(
		parry_stun_shot,
		CombatAnimTransitionsScript.PARRY_CLASH_FADEIN,
		CombatAnimTransitionsScript.PARRY_CLASH_FADEOUT,
		true
	)

	var roll_node := AnimationNodeAnimation.new()
	roll_node.animation = roll_path
	var roll_shot := AnimationNodeOneShot.new()
	CombatAnimTransitionsScript.configure_one_shot(
		roll_shot,
		CombatAnimTransitionsScript.ROLL_FADEIN,
		CombatAnimTransitionsScript.ROLL_FADEOUT
	)

	var blend_tree := AnimationNodeBlendTree.new()
	blend_tree.add_node(UndeadAnimConfigScript.LOCOMOTION_BLEND, locomotion_blend)
	blend_tree.add_node(UndeadAnimConfigScript.BLOCK_BLEND, block_blend)
	blend_tree.add_node(&"ParryPoseAnim", parry_pose_node)
	blend_tree.add_node(&"SwordSlashAnim", sword_slash_node)
	blend_tree.add_node(UndeadAnimConfigScript.SWORD_SLASH_TIME_SCALE, sword_slash_time_scale)
	blend_tree.add_node(UndeadAnimConfigScript.SWORD_SLASH_ONE_SHOT, sword_slash_shot)
	blend_tree.add_node(&"ChargedUpwardAnim", charged_upward_node)
	blend_tree.add_node(UndeadAnimConfigScript.CHARGED_UPWARD_TIME_SCALE, charged_upward_time_scale)
	blend_tree.add_node(UndeadAnimConfigScript.CHARGED_UPWARD_ONE_SHOT, charged_upward_shot)
	blend_tree.add_node(&"SprintSpinAnim", sprint_spin_node)
	blend_tree.add_node(UndeadAnimConfigScript.SPRINT_SPIN_TIME_SCALE, sprint_spin_time_scale)
	blend_tree.add_node(UndeadAnimConfigScript.SPRINT_SPIN_ONE_SHOT, sprint_spin_shot)
	blend_tree.add_node(&"ParryBackwardAnim", parry_backward_node)
	blend_tree.add_node(UndeadAnimConfigScript.PARRY_STUN_ONE_SHOT, parry_stun_shot)
	blend_tree.add_node(&"RollAnim", roll_node)
	blend_tree.add_node(UndeadAnimConfigScript.ROLL_ONE_SHOT, roll_shot)

	blend_tree.connect_node(&"output", 0, UndeadAnimConfigScript.ROLL_ONE_SHOT)
	blend_tree.connect_node(UndeadAnimConfigScript.ROLL_ONE_SHOT, 0, UndeadAnimConfigScript.PARRY_STUN_ONE_SHOT)
	blend_tree.connect_node(UndeadAnimConfigScript.ROLL_ONE_SHOT, 1, &"RollAnim")
	blend_tree.connect_node(UndeadAnimConfigScript.PARRY_STUN_ONE_SHOT, 0, UndeadAnimConfigScript.SPRINT_SPIN_ONE_SHOT)
	blend_tree.connect_node(UndeadAnimConfigScript.PARRY_STUN_ONE_SHOT, 1, &"ParryBackwardAnim")
	blend_tree.connect_node(UndeadAnimConfigScript.SPRINT_SPIN_ONE_SHOT, 0, UndeadAnimConfigScript.CHARGED_UPWARD_ONE_SHOT)
	blend_tree.connect_node(UndeadAnimConfigScript.SPRINT_SPIN_ONE_SHOT, 1, UndeadAnimConfigScript.SPRINT_SPIN_TIME_SCALE)
	blend_tree.connect_node(UndeadAnimConfigScript.SPRINT_SPIN_TIME_SCALE, 0, &"SprintSpinAnim")
	blend_tree.connect_node(UndeadAnimConfigScript.CHARGED_UPWARD_ONE_SHOT, 0, UndeadAnimConfigScript.SWORD_SLASH_ONE_SHOT)
	blend_tree.connect_node(UndeadAnimConfigScript.CHARGED_UPWARD_ONE_SHOT, 1, UndeadAnimConfigScript.CHARGED_UPWARD_TIME_SCALE)
	blend_tree.connect_node(UndeadAnimConfigScript.CHARGED_UPWARD_TIME_SCALE, 0, &"ChargedUpwardAnim")
	blend_tree.connect_node(UndeadAnimConfigScript.SWORD_SLASH_ONE_SHOT, 0, UndeadAnimConfigScript.BLOCK_BLEND)
	blend_tree.connect_node(UndeadAnimConfigScript.SWORD_SLASH_ONE_SHOT, 1, UndeadAnimConfigScript.SWORD_SLASH_TIME_SCALE)
	blend_tree.connect_node(UndeadAnimConfigScript.SWORD_SLASH_TIME_SCALE, 0, &"SwordSlashAnim")
	blend_tree.connect_node(UndeadAnimConfigScript.BLOCK_BLEND, 0, UndeadAnimConfigScript.LOCOMOTION_BLEND)
	blend_tree.connect_node(UndeadAnimConfigScript.BLOCK_BLEND, 1, &"ParryPoseAnim")

	_animation_tree.tree_root = blend_tree
	_animation_tree.anim_player = _animation_tree.get_path_to(_animation_player)
	_animation_tree.active = true
	_animation_tree.set("parameters/LocomotionBlend/blend_position", 0.0)
	_animation_tree.set("parameters/CombatIdleBlend/blend_amount", 0.0)
	_animation_tree.set("parameters/BlockBlend/blend_amount", 0.0)
	_set_attack_playback_speed(ATTACK_PLAYBACK_SPEED)


func _prime_idle_pose() -> void:
	if _animation_player == null:
		return
	var idle_path := String(UndeadAnimUtilsScript.clip_path(UndeadAnimConfigScript.CLIP_IDLE))
	if not _animation_player.has_animation(idle_path):
		return
	if _animation_tree != null:
		_animation_tree.active = true
	_set_locomotion_blend(0.0)


func _play_editor_preview() -> void:
	if not Engine.is_editor_hint() or _animation_player == null:
		return
	var clip := String(editor_preview_animation)
	if not _animation_player.has_animation("%s/%s" % [UndeadAnimConfigScript.LIBRARY, clip]):
		return
	if _animation_tree != null and _animation_tree.active:
		_animation_tree.active = false
	_animation_player.play("%s/%s" % [UndeadAnimConfigScript.LIBRARY, clip])


func _update_combat_target(delta: float) -> void:
	if _combat_target != null and is_instance_valid(_combat_target):
		if not _is_valid_combat_target(_combat_target):
			_combat_target = null
	else:
		_combat_target = null

	# Validity check above stays per tick; the group rescan below runs on the
	# staggered think interval.
	_hostile_scan_accum += delta
	if _hostile_scan_accum < HOSTILE_SCAN_INTERVAL:
		return
	_hostile_scan_accum = 0.0

	if _combat_target != null:
		var nearest := _find_nearest_hostile()
		if (
			nearest != null
			and nearest != _combat_target
			and _should_switch_to_closer_target(nearest)
		):
			_combat_target = nearest
		return

	if prefer_last_attack_target and _last_attack_target != null:
		if _is_valid_combat_target(_last_attack_target):
			_combat_target = _last_attack_target
			return

	_combat_target = _find_nearest_hostile()


func _process_patrol_idle(delta: float) -> void:
	velocity.x = 0.0
	velocity.z = 0.0
	if _combat_target != null:
		_begin_chase()
		return
	_state_timer -= delta
	if _state_timer <= 0.0:
		_begin_patrol_walk()


func _process_patrol_walk(delta: float) -> void:
	if _combat_target != null:
		_begin_chase()
		return
	_move_in_direction(_walk_direction, WALK_SPEED, delta)
	_state_timer -= delta
	if _state_timer <= 0.0:
		_begin_patrol_idle()


func _process_chase(delta: float) -> void:
	if _combat_target == null:
		_begin_patrol_idle()
		return

	var to_target := _combat_target.global_position - global_position
	to_target.y = 0.0
	var distance := to_target.length()
	if distance <= ATTACK_RANGE:
		if randf() < CHASE_BLOCK_CHANCE:
			_begin_blocking(true)
		else:
			_begin_combat_deciding()
		return

	_move_toward(_combat_target.global_position, RUN_SPEED, delta)
	if _combat_nav != null and _combat_nav.is_available():
		_combat_nav.set_target_if_needed(_combat_target.global_position)
		var nav_dir: Vector3 = _combat_nav.get_move_direction(delta)
		if nav_dir.length_squared() > 0.0001:
			_move_in_direction(nav_dir, RUN_SPEED, delta)


func _process_combat_deciding(delta: float) -> void:
	_stop_horizontal_velocity()
	if _combat_target != null:
		_face_position(_combat_target.global_position, delta)

	_decision_timer -= delta
	if _decision_timer > 0.0:
		return

	if _combat_target == null:
		_begin_patrol_idle()
		return

	var to_target := _combat_target.global_position - global_position
	to_target.y = 0.0
	var distance := to_target.length()
	var in_melee := distance <= ATTACK_RANGE + 0.35
	var in_sprint_range := (
		distance >= SPRINT_APPROACH_MIN and distance <= SPRINT_APPROACH_MAX
	)
	if not in_melee and not in_sprint_range:
		_begin_chase()
		return

	var roll := randf()
	if not in_melee and in_sprint_range and _attack_cooldown <= 0.0 and roll < SPRINT_ATTACK_CHANCE:
		_begin_sprint_approach()
		return
	if _attack_cooldown <= 0.0 and roll < ROLL_AWAY_CHANCE:
		var away := global_position - _combat_target.global_position
		away.y = 0.0
		if away.length_squared() < 0.0001:
			away = -_get_flat_forward()
		_begin_roll(away.normalized())
		return
	if _attack_cooldown <= 0.0 and roll < ROLL_AWAY_CHANCE + MELEE_ATTACK_CHANCE:
		_begin_attack_windup()
	elif roll < ROLL_AWAY_CHANCE + MELEE_ATTACK_CHANCE + MELEE_BLOCK_CHANCE:
		_begin_blocking()
	else:
		_pick_random_combat_action()


func _process_blocking(delta: float) -> void:
	_blocking = true
	if _combat_target != null:
		_face_position(_combat_target.global_position, delta)
		if _blocking_approach:
			var to_target := _combat_target.global_position - global_position
			to_target.y = 0.0
			if to_target.length() > ATTACK_RANGE * 0.85:
				_move_in_direction(to_target.normalized(), WALK_SPEED, delta)
			else:
				_stop_horizontal_velocity()
		else:
			_stop_horizontal_velocity()
	else:
		_stop_horizontal_velocity()

	_state_timer -= delta
	if _state_timer <= 0.0:
		_end_blocking()
		_begin_combat_deciding()


func _process_attack_windup(delta: float) -> void:
	_stop_horizontal_velocity()
	if _combat_target != null:
		_face_position(_combat_target.global_position, delta)
		_attack_direction = _get_attack_direction()
	else:
		_attack_direction = _get_attack_direction()

	_windup_timer -= delta * ATTACK_PLAYBACK_SPEED
	if _windup_timer <= 0.0:
		_begin_attacking()


func _process_attacking(delta: float) -> void:
	var attack_delta := delta * ATTACK_PLAYBACK_SPEED
	_attack_elapsed += attack_delta
	_attack_timer -= attack_delta
	_stop_horizontal_velocity()

	if _combat_target != null and is_instance_valid(_combat_target):
		_face_position(_combat_target.global_position, delta)
		_attack_direction = _get_attack_direction()

	var strike_fraction := _get_attack_strike_fraction()
	var strike_time := _get_attack_length() * strike_fraction
	if not _attack_struck and _attack_elapsed >= strike_time:
		_attack_struck = true
		if _combat_target != null and is_instance_valid(_combat_target):
			_last_attack_target = _combat_target
		var strike_dir := _get_attack_direction()
		var attack_kind_name := _get_attack_kind_name()
		UndeadMeleeStrikeScript.play_strike_presentation(
			self,
			strike_dir,
			_combat_target,
			attack_kind_name
		)
		UndeadMeleeStrikeScript.apply_strike(
			self,
			strike_dir,
			_combat_target,
			_attack_kind == AttackKind.SPRINT_SPIN
		)

	if _attack_timer <= 0.0:
		_end_attacking()


func _process_rolling(delta: float) -> void:
	_roll_timer -= delta
	_move_in_direction(_roll_direction, ROLL_SPEED, delta)
	if _roll_timer <= 0.0:
		_end_rolling()


func _process_sprint_approach(delta: float) -> void:
	if _combat_target == null:
		_begin_patrol_idle()
		return

	var to_target := _combat_target.global_position - global_position
	to_target.y = 0.0
	var distance := to_target.length()
	if distance <= ATTACK_RANGE + 0.5:
		_begin_attack_windup(AttackKind.SPRINT_SPIN)
		return

	_move_toward(_combat_target.global_position, RUN_SPEED, delta)
	if _combat_nav != null and _combat_nav.is_available():
		_combat_nav.set_target_if_needed(_combat_target.global_position)
		var nav_dir: Vector3 = _combat_nav.get_move_direction(delta)
		if nav_dir.length_squared() > 0.0001:
			_move_in_direction(nav_dir, RUN_SPEED, delta)


func _process_relocating(delta: float) -> void:
	var to_target := _relocate_target - global_position
	to_target.y = 0.0
	if to_target.length() <= RELOCATE_ARRIVE_DIST:
		_begin_combat_deciding()
		return
	_move_in_direction(to_target.normalized(), RUN_SPEED, delta)


func _process_parry_stunned(delta: float) -> void:
	_state_timer -= delta
	if _state_timer > 0.0:
		return

	_melee_stun_timer = 0.0
	_abort_parry_stun_anim()
	_finish_parry_followup()


func _begin_patrol_idle() -> void:
	_ai_state = AiState.PATROL_IDLE
	_state_timer = randf_range(PATROL_IDLE_MIN, PATROL_IDLE_MAX)
	_walk_direction = Vector3.ZERO
	_set_combat_idle_blend(0.0)
	_tween_block_blend(0.0, CombatAnimTransitionsScript.BLOCK_HOLD_BLEND_OUT)
	_blocking = false


func _begin_patrol_walk() -> void:
	_ai_state = AiState.PATROL_WALK
	_state_timer = randf_range(PATROL_WALK_MIN, PATROL_WALK_MAX)
	var angle := randf_range(0.0, TAU)
	_walk_direction = Vector3(sin(angle), 0.0, cos(angle)).normalized()
	_clamp_walk_to_roam()


func _begin_chase() -> void:
	_ai_state = AiState.CHASE
	_set_combat_idle_blend(1.0)
	_tween_block_blend(0.0, CombatAnimTransitionsScript.BLOCK_HOLD_BLEND_OUT)
	_blocking = false


func _begin_sprint_approach() -> void:
	_ai_state = AiState.SPRINT_APPROACH
	_set_combat_idle_blend(1.0)
	_tween_block_blend(0.0, CombatAnimTransitionsScript.BLOCK_HOLD_BLEND_OUT)
	_blocking = false


func _begin_combat_deciding() -> void:
	_ai_state = AiState.COMBAT_DECIDING
	_set_combat_idle_blend(1.0)
	if _combat_target != null:
		var to_target := _combat_target.global_position - global_position
		to_target.y = 0.0
		var distance := to_target.length()
		var in_melee := distance <= ATTACK_RANGE + 0.35
		if in_melee:
			_decision_timer = randf_range(MELEE_DECISION_MIN, MELEE_DECISION_MAX)
		else:
			_decision_timer = randf_range(DECISION_MIN, DECISION_MAX)
	else:
		_decision_timer = randf_range(DECISION_MIN, DECISION_MAX)
	_tween_block_blend(0.0, CombatAnimTransitionsScript.BLOCK_HOLD_BLEND_OUT)
	_blocking = false


func _begin_blocking(approach := false, from_parry := false, hold_duration := -1.0) -> void:
	_ai_state = AiState.BLOCKING
	if hold_duration > 0.0:
		_state_timer = hold_duration
	elif from_parry:
		_state_timer = randf_range(BLOCK_DURATION_MIN, BLOCK_DURATION_MAX)
	elif approach:
		_state_timer = randf_range(BLOCK_DURATION_MIN, BLOCK_DURATION_MAX)
	else:
		_state_timer = randf_range(BLOCK_DURATION_MIN, BLOCK_DURATION_MAX)
	_blocking = true
	_blocking_approach = approach
	_locomotion_blend = 0.0
	_set_locomotion_blend(0.0)
	_tween_block_blend(1.0, CombatAnimTransitionsScript.BLOCK_HOLD_BLEND_IN)


func _begin_attack_windup(preferred_kind: AttackKind = AttackKind.SWORD_SLASH) -> void:
	_ai_state = AiState.ATTACK_WINDUP
	var in_melee := false
	if _combat_target != null:
		var to_target := _combat_target.global_position - global_position
		to_target.y = 0.0
		in_melee = to_target.length() <= ATTACK_RANGE + 0.35

	if preferred_kind == AttackKind.SPRINT_SPIN:
		_attack_kind = AttackKind.SPRINT_SPIN
		_windup_timer = randf_range(
			UndeadMeleeStrikeScript.SPRINT_WINDUP_MIN,
			UndeadMeleeStrikeScript.SPRINT_WINDUP_MAX
		)
	elif in_melee:
		if preferred_kind == AttackKind.CHARGED_UPWARD:
			_attack_kind = AttackKind.CHARGED_UPWARD
		elif randf() < CHARGED_ATTACK_CHANCE:
			_attack_kind = AttackKind.CHARGED_UPWARD
		else:
			_attack_kind = AttackKind.SWORD_SLASH
		_windup_timer = randf_range(0.35, 0.85)
	else:
		_attack_kind = AttackKind.SWORD_SLASH
		_windup_timer = randf_range(UndeadMeleeStrikeScript.WINDUP_MIN, UndeadMeleeStrikeScript.WINDUP_MAX)
	_tween_block_blend(0.0, CombatAnimTransitionsScript.BLOCK_HOLD_BLEND_OUT)
	_blocking = false


func _begin_attacking() -> void:
	_ai_state = AiState.ATTACKING
	_attack_elapsed = 0.0
	_attack_timer = _get_attack_length()
	_attack_struck = false
	if _attack_kind == AttackKind.SPRINT_SPIN:
		_attack_cooldown = UndeadMeleeStrikeScript.SPRINT_COOLDOWN
	else:
		_attack_cooldown = UndeadMeleeStrikeScript.COOLDOWN
	if _animation_tree != null:
		var one_shot_name := _get_attack_one_shot_name()
		_animation_tree.set(
			"parameters/%s/request" % one_shot_name,
			AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE
		)


func _end_attacking() -> void:
	_attack_struck = false
	_begin_post_attack_disengage()


func _begin_post_attack_disengage() -> void:
	if _combat_target == null:
		_begin_patrol_idle()
		return

	if randf() < 0.4:
		var refreshed := _find_nearest_hostile()
		if refreshed != null:
			_combat_target = refreshed

	if randf() < 0.25:
		var away := global_position - _combat_target.global_position
		away.y = 0.0
		if away.length_squared() < 0.0001:
			away = -_get_flat_forward()
		_begin_roll(away.normalized())
	else:
		_begin_relocate()


func _update_player_gun_aim_threat(delta: float) -> void:
	if _gun_aim_roll_committed:
		return

	# Player lookup + aim capsule test think on an interval; the accumulated
	# delta keeps the roll-delay countdown at real-time pace.
	_gun_aim_scan_accum += delta
	if _gun_aim_scan_accum < GUN_AIM_SCAN_INTERVAL:
		return
	var scan_delta := _gun_aim_scan_accum
	_gun_aim_scan_accum = 0.0

	var player := _find_player()
	var aimed := player != null and _is_player_pointing_gun_at_me(player)
	if aimed:
		_focus_hostile(player)
		if _gun_aim_roll_cooldown > 0.0:
			return
		if not _gun_aim_roll_pending:
			_gun_aim_roll_pending = true
			_gun_aim_roll_timer = randf_range(GUN_AIM_ROLL_DELAY_MIN, GUN_AIM_ROLL_DELAY_MAX)
			_gun_aim_roll_threat = player
			return

		_gun_aim_roll_timer -= scan_delta
		if _gun_aim_roll_timer <= 0.0:
			_gun_aim_roll_pending = false
			_gun_aim_roll_committed = true
		return

	if _gun_aim_roll_pending:
		_clear_gun_aim_roll_pending()


func _clear_gun_aim_roll_pending() -> void:
	_gun_aim_roll_pending = false
	_gun_aim_roll_timer = 0.0
	if not _gun_aim_roll_committed:
		_gun_aim_roll_threat = null


func _clear_gun_aim_roll_response() -> void:
	_clear_gun_aim_roll_pending()
	_gun_aim_roll_committed = false
	_gun_aim_roll_threat = null


func _can_perform_gun_aim_roll() -> bool:
	if _defeated or _gun_aim_roll_cooldown > 0.0:
		return false
	if _ai_state in [
		AiState.ROLLING,
		AiState.ATTACK_WINDUP,
		AiState.ATTACKING,
		AiState.PARRY_STUNNED,
		AiState.SPRINT_APPROACH,
	]:
		return false
	return true


func _try_execute_committed_gun_aim_roll() -> void:
	if not _gun_aim_roll_committed or not _can_perform_gun_aim_roll():
		return
	if _gun_aim_roll_threat == null or not is_instance_valid(_gun_aim_roll_threat):
		_clear_gun_aim_roll_response()
		return

	_gun_aim_roll_committed = false
	_gun_aim_roll_cooldown = GUN_AIM_ROLL_COOLDOWN
	if _blocking:
		_end_blocking()
	_begin_roll(_get_random_roll_direction())
	_gun_aim_roll_threat = null


func _begin_roll(direction: Vector3) -> void:
	_ai_state = AiState.ROLLING
	_roll_direction = direction
	if _roll_direction.length_squared() < 0.0001:
		_roll_direction = _get_flat_forward()
	_roll_duration = _get_clip_length(UndeadAnimConfigScript.CLIP_ROLL_DODGE, 0.7)
	_roll_timer = _roll_duration
	if _animation_tree != null:
		_animation_tree.set(
			"parameters/%s/request" % UndeadAnimConfigScript.ROLL_ONE_SHOT,
			AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE
		)


func _end_rolling() -> void:
	_begin_combat_deciding()


func _begin_relocate() -> void:
	_ai_state = AiState.RELOCATING
	_relocate_target = _pick_relocate_point()
	if _relocate_target.distance_squared_to(global_position) < 0.25:
		_begin_combat_deciding()


func _pick_random_combat_action() -> void:
	if _combat_target == null:
		_begin_patrol_idle()
		return

	var roll := randf()
	if roll < 0.14:
		_begin_relocate()
	elif roll < 0.24:
		var away := global_position - _combat_target.global_position
		away.y = 0.0
		if away.length_squared() < 0.0001:
			away = -_get_flat_forward()
		_begin_roll(away.normalized())
	elif roll < 0.82 and _attack_cooldown <= 0.0:
		_begin_attack_windup()
	elif roll < 0.92:
		_begin_blocking()
	else:
		_begin_combat_deciding()


func on_melee_clash_blocked(
	_attacker: Node,
	_hit_info: Dictionary,
	stun_duration: float
) -> void:
	hold_knockback_velocity(CombatKnockbackScript.DEFAULT_HOLD)
	_enter_parry_clash_stun(stun_duration)


func on_melee_clash_attacker(
	_defender: Node,
	_hit_info: Dictionary,
	stun_duration: float
) -> void:
	hold_knockback_velocity(CombatKnockbackScript.DEFAULT_HOLD)
	if _ai_state in [AiState.ATTACKING, AiState.ATTACK_WINDUP]:
		_attack_struck = true
		_attack_timer = 0.0
		_abort_attack_one_shot()
	_enter_parry_clash_stun(stun_duration)


func _abort_attack_one_shot() -> void:
	if _animation_tree == null or not _animation_tree.active:
		return
	for one_shot_name: StringName in [
		UndeadAnimConfigScript.SWORD_SLASH_ONE_SHOT,
		UndeadAnimConfigScript.CHARGED_UPWARD_ONE_SHOT,
		UndeadAnimConfigScript.SPRINT_SPIN_ONE_SHOT,
	]:
		_animation_tree.set(
			"parameters/%s/request" % one_shot_name,
			AnimationNodeOneShot.ONE_SHOT_REQUEST_ABORT
		)


func _enter_parry_clash_stun(stun_duration: float) -> void:
	_ai_state = AiState.PARRY_STUNNED
	_state_timer = stun_duration
	_melee_stun_timer = stun_duration
	_blocking = false
	_blocking_approach = false
	_locomotion_blend = 0.0

	if _animation_tree != null:
		_set_locomotion_blend(0.0)
		_tween_block_blend(0.0, CombatAnimTransitionsScript.CLASH_BLOCK_BLEND_OUT)
		_animation_tree.set(
			"parameters/%s/request" % UndeadAnimConfigScript.PARRY_STUN_ONE_SHOT,
			AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE
		)


func _finish_parry_followup() -> void:
	if _combat_target == null:
		_begin_patrol_idle()
		return

	var roll := randf()
	if roll < PARRY_FOLLOWUP_ATTACK_CHANCE:
		var to_target := _combat_target.global_position - global_position
		to_target.y = 0.0
		if to_target.length() > ATTACK_RANGE + 0.35:
			_begin_chase()
		elif _attack_cooldown <= 0.0:
			_begin_attack_windup()
		else:
			_begin_chase()
		return

	if roll < PARRY_FOLLOWUP_ATTACK_CHANCE + PARRY_FOLLOWUP_REBLOCK_CHANCE:
		_begin_blocking(
			false,
			true,
			randf_range(PARRY_REBLOCK_HOLD_MIN, PARRY_REBLOCK_HOLD_MAX)
		)
		return

	_begin_post_block_reposition()


func _begin_post_block_reposition() -> void:
	if _combat_target == null:
		_begin_combat_deciding()
		return

	if randf() < 0.5:
		var refreshed := _find_nearest_hostile()
		if refreshed != null:
			_combat_target = refreshed

	if randf() < 0.5:
		var away := global_position - _combat_target.global_position
		away.y = 0.0
		if away.length_squared() < 0.0001:
			away = _get_strafe_roll_direction()
		_begin_roll(away.normalized())
	else:
		_begin_relocate()


func _abort_parry_stun_anim() -> void:
	if _animation_tree == null or not _animation_tree.active:
		return
	_animation_tree.set(
		"parameters/%s/request" % UndeadAnimConfigScript.PARRY_STUN_ONE_SHOT,
		AnimationNodeOneShot.ONE_SHOT_REQUEST_ABORT
	)


func _is_valid_combat_target(node: Node) -> bool:
	if not _is_valid_hostile(node):
		return false
	var offset := (node as Node3D).global_position - global_position
	offset.y = 0.0
	return offset.length_squared() <= sight_range * sight_range * 1.35


func _focus_hostile(target: Node3D) -> void:
	if target == null or not _is_valid_hostile(target):
		return
	_combat_target = target
	if _ai_state in [AiState.PATROL_IDLE, AiState.PATROL_WALK]:
		_begin_chase()


func _should_switch_to_closer_target(nearest: Node3D) -> bool:
	if _combat_target == null:
		return true
	var current_offset := _combat_target.global_position - global_position
	var nearest_offset := nearest.global_position - global_position
	current_offset.y = 0.0
	nearest_offset.y = 0.0
	var current_dist_sq := current_offset.length_squared()
	var nearest_dist_sq := nearest_offset.length_squared()
	if nearest_dist_sq < current_dist_sq * 0.64:
		return true
	if prefer_last_attack_target and nearest == _last_attack_target:
		return true
	return false


func _reset_combat_anim_overlays() -> void:
	_set_block_blend(0.0)
	if _animation_tree == null or not _animation_tree.active:
		return
	for one_shot_name: StringName in [
		UndeadAnimConfigScript.PARRY_STUN_ONE_SHOT,
		UndeadAnimConfigScript.SWORD_SLASH_ONE_SHOT,
		UndeadAnimConfigScript.CHARGED_UPWARD_ONE_SHOT,
		UndeadAnimConfigScript.SPRINT_SPIN_ONE_SHOT,
		UndeadAnimConfigScript.ROLL_ONE_SHOT,
	]:
		_animation_tree.set(
			"parameters/%s/request" % one_shot_name,
			AnimationNodeOneShot.ONE_SHOT_REQUEST_ABORT
		)


func _die(hit_info: Dictionary) -> void:
	if _defeated:
		return
	var hit_position: Vector3 = hit_info.get("position", global_position + Vector3(0.0, 1.0, 0.0))
	GameAudioScript.play_death_sound(self, hit_position)
	BloodSplatterFXScript.spawn_big_for_hit(self, hit_info)
	_defeated = true
	velocity = Vector3.ZERO
	_bind_rig()
	if _ragdoll != null and _skeleton != null:
		_ragdoll.skeleton_path = _ragdoll.get_path_to(_skeleton)
		if _model != null:
			_ragdoll.model_path = _ragdoll.get_path_to(_model)
		_ragdoll.bind_skeleton()
	if _ragdoll != null and not _ragdoll.is_active():
		_suspend_locomotion_animations()
		_ragdoll.activate(hit_info, _animation_player)


func _find_nearest_hostile() -> Node3D:
	var best: Node3D = null
	var best_dist_sq := sight_range * sight_range
	for group_name: StringName in [&"overworld_player", &"crusader_npc"]:
		for node in get_tree().get_nodes_in_group(group_name):
			if not (node is Node3D):
				continue
			if not _is_valid_hostile(node):
				continue
			var offset := (node as Node3D).global_position - global_position
			offset.y = 0.0
			var dist_sq := offset.length_squared()
			if dist_sq < best_dist_sq:
				best_dist_sq = dist_sq
				best = node
	return best


func _pick_relocate_point() -> Vector3:
	var center := global_position
	if _combat_target != null:
		center = _combat_target.global_position
	var angle := randf_range(0.0, TAU)
	var distance := randf_range(3.5, 7.5)
	var offset := Vector3(sin(angle), 0.0, cos(angle)) * distance
	return center + offset


func _get_attack_length() -> float:
	match _attack_kind:
		AttackKind.CHARGED_UPWARD:
			return _get_clip_length(UndeadAnimConfigScript.CLIP_CHARGED_UPWARD, 3.2)
		AttackKind.SPRINT_SPIN:
			return _get_clip_length(UndeadAnimConfigScript.CLIP_SPRINT_SPIN, 1.8)
		_:
			return _get_clip_length(UndeadAnimConfigScript.CLIP_SWORD_SLASH, 1.5)


func _get_attack_strike_fraction() -> float:
	match _attack_kind:
		AttackKind.CHARGED_UPWARD:
			return UndeadMeleeStrikeScript.CHARGED_UPWARD_STRIKE_FRACTION
		AttackKind.SPRINT_SPIN:
			return UndeadMeleeStrikeScript.SPRINT_SPIN_STRIKE_FRACTION
		_:
			return UndeadMeleeStrikeScript.SWORD_SLASH_STRIKE_FRACTION


func _get_attack_direction() -> Vector3:
	return UndeadMeleeStrikeScript.get_strike_direction(self, _combat_target)


func _set_attack_playback_speed(speed: float) -> void:
	if _animation_tree == null or not _animation_tree.active:
		return
	for time_scale_node: StringName in [
		UndeadAnimConfigScript.SWORD_SLASH_TIME_SCALE,
		UndeadAnimConfigScript.CHARGED_UPWARD_TIME_SCALE,
		UndeadAnimConfigScript.SPRINT_SPIN_TIME_SCALE,
	]:
		_animation_tree.set("parameters/%s/scale" % time_scale_node, speed)


func _get_attack_one_shot_name() -> StringName:
	match _attack_kind:
		AttackKind.CHARGED_UPWARD:
			return UndeadAnimConfigScript.CHARGED_UPWARD_ONE_SHOT
		AttackKind.SPRINT_SPIN:
			return UndeadAnimConfigScript.SPRINT_SPIN_ONE_SHOT
		_:
			return UndeadAnimConfigScript.SWORD_SLASH_ONE_SHOT


func _get_attack_kind_name() -> StringName:
	match _attack_kind:
		AttackKind.CHARGED_UPWARD:
			return &"charged_upward"
		AttackKind.SPRINT_SPIN:
			return &"sprint_spin"
		_:
			return &"sword_slash"


func _update_combat_idle_blend(delta: float) -> void:
	var target := 1.0 if _is_in_combat() else 0.0
	_combat_idle_blend = lerpf(_combat_idle_blend, target, BLEND_SPEED * delta)
	_set_combat_idle_blend(_combat_idle_blend)


func _is_in_combat() -> bool:
	return _ai_state in [
		AiState.CHASE,
		AiState.COMBAT_DECIDING,
		AiState.BLOCKING,
		AiState.ATTACK_WINDUP,
		AiState.ATTACKING,
		AiState.RELOCATING,
		AiState.PARRY_STUNNED,
		AiState.SPRINT_APPROACH,
	]


func _set_combat_idle_blend(value: float) -> void:
	if _animation_tree == null or not _animation_tree.active:
		return
	_animation_tree.set(
		"parameters/LocomotionBlend/CombatIdleBlend/blend_amount",
		clampf(value, 0.0, 1.0)
	)


func _get_clip_length(clip_name: StringName, fallback: float) -> float:
	if _animation_player == null:
		return fallback
	var clip_path := String(UndeadAnimUtilsScript.clip_path(clip_name))
	if _animation_player.has_animation(clip_path):
		return _animation_player.get_animation(clip_path).length
	return fallback


func _update_locomotion_blend(delta: float, horizontal_speed: float) -> void:
	var target := 0.0
	if horizontal_speed > LOCOMOTION_STOP_SPEED:
		target = 1.0 if horizontal_speed >= LOCOMOTION_RUN_SPEED else 0.5
	_locomotion_blend = lerpf(_locomotion_blend, target, BLEND_SPEED * delta)
	_set_locomotion_blend(_locomotion_blend)


func _clamp_walk_to_roam() -> void:
	var next_pos := global_position + _walk_direction * WALK_SPEED * PATROL_WALK_MAX
	var offset := next_pos - _roam_center
	offset.y = 0.0
	if offset.length() > roam_radius:
		_walk_direction = (_roam_center - global_position).normalized()
		_walk_direction.y = 0.0
