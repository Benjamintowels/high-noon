...
var _active_talk_state := UncleToadAnimConfig.IDLE_DEFAULT
var _idle_emote_timer := 0.0
var _idle_funky_delay := UncleToadAnimConfig.FUNKY_EMOTE_DELAY_MIN
...
func _physics_process(delta: float) -> void:
	...
	match _ai_state:
		AiState.IDLE:
			velocity.x = 0.0
			velocity.z = 0.0
			if not _talking and _state_timer <= 0.0:
				_try_begin_social_talk()
			if not _talking and _ai_state == AiState.IDLE and _state_timer <= 0.0:
				_begin_walk()
		AiState.WALKING:
...
func alert_to_gunshot(origin: Vector3) -> void:
	if _defeated or _talking or _lasso_captured:
		return
...
func _update_player_weapon_reaction(_delta: float) -> void:
	if _defeated or _talking or _lasso_captured or _ai_state == AiState.SOCIAL_TALK:
		return
...
func _begin_flee(origin: Vector3) -> void:
	if _defeated or _talking or _lasso_captured:
		return
...
func _setup_locomotion() -> void:
	if _animation_player == null:
		push_error("CivilianNpc: missing AnimationPlayer on UncleToad body.")
		return
	...
	for state_name: StringName in UncleToadAnimConfig.IDLE_CLIP_BY_STATE.keys():
		var meshy_clip: StringName = UncleToadAnimConfig.IDLE_CLIP_BY_STATE[state_name]
		var loop_once := state_name in UncleToadAnimConfig.LOOP_ONCE_STATES
		_add_meshy_clip(library, state_name, meshy_clip, not loop_once)
	_add_meshy_clip(library, UncleToadAnimConfig.LOCOMOTION_WALK, UncleToadAnimConfig.MESHY_WALK)
	_add_meshy_clip(library, UncleToadAnimConfig.LOCOMOTION_RUN, UncleToadAnimConfig.MESHY_RUN)

	if _animation_player.has_animation_library(UncleToadAnimConfig.LOCOMOTION_LIBRARY):
		_animation_player.remove_animation_library(UncleToadAnimConfig.LOCOMOTION_LIBRARY)
	_animation_player.add_animation_library(UncleToadAnimConfig.LOCOMOTION_LIBRARY, library)

	var idle_paths := _required_anim_paths(
		[
			UncleToadAnimConfig.IDLE_DEFAULT,
			UncleToadAnimConfig.IDLE_FUNKY,
			UncleToadAnimConfig.IDLE_TALK_PASSION,
			UncleToadAnimConfig.IDLE_TALK_HANDS_OPEN,
			UncleToadAnimConfig.IDLE_TALK_HAND_RAISED,
		]
	)
	var walk_path := _anim_path(UncleToadAnimConfig.LOCOMOTION_WALK)
	var run_path := _anim_path(UncleToadAnimConfig.LOCOMOTION_RUN)
	if idle_paths.is_empty() or not _animation_player.has_animation(walk_path):
		push_error("CivilianNpc: locomotion clips missing on AnimationPlayer.")
		return
	...
	blend_tree.add_node(UncleToadAnimConfig.IDLE_STATE_NODE, idle_sm)
	blend_tree.add_node(UncleToadAnimConfig.LOCOMOTION_BLEND_NODE, walk_run_space)
	blend_tree.add_node(UncleToadAnimConfig.MOVE_BLEND_NODE, move_blend)
	blend_tree.connect_node(UncleToadAnimConfig.MOVE_BLEND_NODE, 0, UncleToadAnimConfig.IDLE_STATE_NODE)
	blend_tree.connect_node(UncleToadAnimConfig.MOVE_BLEND_NODE, 1, UncleToadAnimConfig.LOCOMOTION_BLEND_NODE)
	blend_tree.connect_node(&"output", 0, UncleToadAnimConfig.MOVE_BLEND_NODE)
...
func _required_anim_paths(clip_names: Array) -> Dictionary:
	...
	for clip_name: StringName in clip_names:
		...
		if not _animation_player.has_animation(path):
			push_error("CivilianNpc: missing clip '%s'." % clip_name)
			return {}
...
func _anim_path(clip_name: StringName) -> StringName:
	return StringName(
		"%s/%s" % [UncleToadAnimConfig.LOCOMOTION_LIBRARY, clip_name]
	)
...
func _build_idle_state_machine(idle_paths: Dictionary) -> AnimationNodeStateMachine:
	...
	idle_sm.add_transition(&"Start", UncleToadAnimConfig.IDLE_DEFAULT, start_transition)

	var funky_to_default := AnimationNodeStateMachineTransition.new()
	funky_to_default.xfade_time = UncleToadAnimConfig.IDLE_CROSSFADE
	funky_to_default.switch_mode = AnimationNodeStateMachineTransition.SwitchMode.SWITCH_MODE_AT_END
	idle_sm.add_transition(
		UncleToadAnimConfig.IDLE_FUNKY,
		UncleToadAnimConfig.IDLE_DEFAULT,
		funky_to_default
	)
...
func _add_meshy_clip(
	...
	var raw := RigAnimUtils.load_skeleton_animation(UncleToadAnimConfig.MERGED_SCENE, meshy_clip)
	if raw == null:
		push_error("CivilianNpc: failed to load clip '%s'." % meshy_clip)
		return
...
func _update_locomotion_blend(delta: float, speed: float, sprinting: bool) -> void:
	...
	var moving := speed > 0.05 and _ai_state != AiState.SOCIAL_TALK and _ai_state != AiState.TALKING
	if moving:
		...
		_set_move_blend(0.0)
		_set_walk_run_blend(0.0)
		if not _is_in_talk_pose() and _active_talk_state != UncleToadAnimConfig.IDLE_FUNKY:
			_reset_idle_anim()
...
func _update_idle_emote(delta: float) -> void:
	...
	var current_idle := _current_idle_state()
	if current_idle != &"":
		_active_talk_state = current_idle

	if _move_blend > 0.05 or _ai_state == AiState.FLEEING:
		_idle_emote_timer = 0.0
		return
	if _ai_state not in [AiState.IDLE, AiState.STARING]:
		return
	if _is_in_talk_pose() or _active_talk_state == UncleToadAnimConfig.IDLE_FUNKY:
		return

	_idle_emote_timer += delta
	if _idle_emote_timer >= _idle_funky_delay:
		_idle_emote_timer = 0.0
		_idle_funky_delay = randf_range(
			UncleToadAnimConfig.FUNKY_EMOTE_DELAY_MIN,
			UncleToadAnimConfig.FUNKY_EMOTE_DELAY_MAX
		)
		_travel_idle_state(UncleToadAnimConfig.IDLE_FUNKY)
...
func _set_move_blend(value: float) -> void:
	if _animation_tree == null:
		return
	_animation_tree.set(
		"parameters/%s/blend_amount" % UncleToadAnimConfig.MOVE_BLEND_NODE,
		value
	)
...
func _set_walk_run_blend(value: float) -> void:
	if _animation_tree == null:
		return
	_animation_tree.set(
		"parameters/%s/blend_position" % UncleToadAnimConfig.LOCOMOTION_BLEND_NODE,
		value
	)
...
func _reset_idle_anim() -> void:
	...
	_active_talk_state = UncleToadAnimConfig.IDLE_DEFAULT
	_travel_idle_state(UncleToadAnimConfig.IDLE_DEFAULT)


func _is_in_talk_pose() -> bool:
	return (
		_active_talk_state != UncleToadAnimConfig.IDLE_DEFAULT
		and _active_talk_state != UncleToadAnimConfig.IDLE_FUNKY
	)
...
func _travel_idle_state(state_name: StringName) -> void:
	...
	var playback: AnimationNodeStateMachinePlayback = _animation_tree.get(
		"parameters/%s/playback" % UncleToadAnimConfig.IDLE_STATE_NODE
	)
	if playback == null:
		return
	if playback.get_current_node() == state_name:
		_active_talk_state = state_name
		return
...
func _play_social_talk_anim() -> void:
	var meshy_clip := UncleToadAnimConfig.MESHY_SOCIAL_TALK_CLIPS[
		randi() % UncleToadAnimConfig.MESHY_SOCIAL_TALK_CLIPS.size()
	]
	var state_name: StringName = UncleToadAnimConfig.TALK_STATE_BY_MESHY.get(
		meshy_clip,
		UncleToadAnimConfig.IDLE_TALK_HANDS_OPEN
	)
	_set_move_blend(0.0)
	_travel_idle_state(state_name)
...
func _begin_dialog_talk_anim() -> void:
	_play_social_talk_anim()


func _end_dialog_talk_anim() -> void:
	_stop_talk_anim()
...
func _play_voice(stream: AudioStream) -> void:
	...
	_voice_player.name = "CivilianVoice"
...
func _play_woah_voice() -> void:
	_play_voice(UncleToadAudio.pick_woah_voice())


func _play_talk_voice() -> void:
	_play_voice(UncleToadAudio.pick_talk_voice())


func _play_scream_voice() -> void:
	_play_voice(UncleToadAudio.SCREAM)
...
func _set_dialog_talking(active: bool, player: Node3D = null) -> void:
	_talking = active
	if active:
		_saved_ai_state = _ai_state
		...
		if player != null:
			_aim_target = player
		_begin_dialog_talk_anim()
	elif _ai_state == AiState.TALKING:
		_end_dialog_talk_anim()
		_ai_state = AiState.IDLE
		_aim_target = null
		_begin_idle()


func _end_player_dialog(player: Node3D) -> void:
	_set_dialog_talking(false)
	_stop_voice()
...
func _activate_defeat_ragdoll(hit_info: Dictionary) -> void:
	...
	if _talking:
		...
		if _player_in_range != null:
			_end_player_dialog(_player_in_range)
...
func _setup_lasso_ragdoll() -> void:
	if _skeleton == null:
		push_error("CivilianNpc: missing skeleton for lasso ragdoll.")
		return
