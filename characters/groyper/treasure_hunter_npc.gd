...
func interact(player: Node3D) -> void:
	...
	if PinkTreeTreasureQuest.accepted:
...
func _show_intro_dialog(player: Node3D) -> void:
	DialogManager.show_dialog_sequence(
		PackedStringArray([
			"Psst — I know where there's buried treasure.",
			"Help me dig it up and we'll split it fifty-fifty.",
		]),
		func() -> void:
			DialogManager.show_choices(
				PackedStringArray(["Deal", "No thanks"]),
				func(choice_index: int) -> void:
					if choice_index == 0:
						_on_player_accepted_deal(player)
					else:
...
func _on_player_accepted_deal(player: Node3D) -> void:
	DialogManager.show_dialog_sequence(
		PackedStringArray(["righty too then take this"]),
		func() -> void:
			_grant_quest_shovel()
			PinkTreeTreasureQuest.begin_quest()
			_end_dialog(player),
		speaker_name,
		func(_line_index: int) -> void:
			_play_gropyptalk()
...
func _show_hint_dialog(player: Node3D) -> void:
	DialogManager.show_dialog_sequence(
		PackedStringArray([
			"Dont forget that shovel, youll need it",
			"It was by the pink tree i dunno",
		]),
		func() -> void:
			_end_dialog(player),
		speaker_name,
...
func _grant_quest_shovel() -> void:
	if PlayerInventory.owns_weapon_type(GroyperWeapons.Id.SHOVEL):
		return

	var pickup = WEAPON_PICKUP_SCENE.instantiate()
	pickup.weapon_id = GroyperWeapons.Id.SHOVEL
...
func _play_gropyptalk() -> void:
	...
	_voice_player.name = "TreasureHunterVoice"
...
func _setup_crown() -> void:
	...
	if mount == null:
		push_warning("TreasureHunterNpc: missing CowboyHatMount on skeleton.")
		return

	var hat_offset := mount.get_node_or_null("HatOffset") as Node3D
	if hat_offset == null:
		push_warning("TreasureHunterNpc: missing HatOffset on hat mount.")
		return
...
func _setup_locomotion() -> void:
	if _animation_player == null:
		push_error("TreasureHunterNpc: missing AnimationPlayer on groyper body.")
		return
	...
	if not _animation_player.has_animation(idle_path) or not _animation_player.has_animation(walk_path):
		push_error("TreasureHunterNpc: locomotion clips missing on AnimationPlayer.")
		return
...
func _add_locomotion_clip(
	...
	if raw == null:
		push_error(
			"TreasureHunterNpc: failed to load locomotion clip '%s' from %s."
