...
- `stages/` — `stage1/` is the Story Mode overworld (one big scene, `stage1.gd` spawns most dynamic content against hardcoded node paths — FROZEN: new content goes zone-local or in roguelike scenes), `caves/` dungeon, `hubworld/` roguelike hub town, `runs/` roguelike run zones (shared `run_zone.gd`), `baldwin_melee_test/` dev stage. Roguelike session/run state lives in `RunState` — it must NEVER write to adventure_save.json; hub/meta progress persists via `RoguelikeSave` (`user://roguelike_save.json`). Run gates use `gameplay/world/run_gate.gd` (walk + fade + scene swap); roguelike death routes to the hub via the `RunState.roguelike_active` branch in the player's `_on_death_cinematic_complete`. Death strips carried weapons; victory keeps one via extract prompt; hub weapon chest stores keepers on the save.
...
- `docs/adding-elemental-gems.md` — elemental gem system + checklist for a new gem (catalog, stamina, on-hit wiring, FX).
