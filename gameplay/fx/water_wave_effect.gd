...
var _player_knockback_speed := 4.2
...
func _physics_process(delta: float) -> void:
	...
	if _segment_timer <= 0.0:
		...
		WaterWaveFXScript.spawn_segment(fx_parent, global_position, _direction)
...
func _try_topple_pillars() -> void:
	...
	for node in tree.get_nodes_in_group(&"PillarFall"):
		...
		if pillar.has_method("pillar_fall") and pillar.has_method("get") and pillar.get("fallen"):
			if pillar.get("fallen"):
				continue
		var offset := pillar.global_position - global_position
		offset.y = 0.0
		if offset.length_squared() <= (_half_width + 1.5) * (_half_width + 1.5):
			if pillar.has_method("pillar_fall"):
				pillar.pillar_fall(6000)
