...
static func _flash_three_tone(
	...
	for mesh: MeshInstance3D in visual_root.find_children("*", "MeshInstance3D", true, false):
		if _should_skip_mesh(mesh):
			continue
		_flash_mesh_three_tone(actor, mesh, first_color, second_color, third_color, tree)
...
static func _flash_mesh_three_tone(
	...
	tween.set_parallel(true)
	for flash_mat: StandardMaterial3D in flash_materials:
		tween.tween_property(flash_mat, "albedo_color", second_color, BLOCK_BREAK_STEP)
		tween.tween_property(flash_mat, "emission", second_color * 0.4, BLOCK_BREAK_STEP)
	tween.set_parallel(false)
	tween.set_parallel(true)
	for flash_mat: StandardMaterial3D in flash_materials:
		tween.tween_property(flash_mat, "albedo_color", third_color, BLOCK_BREAK_STEP)
		tween.tween_property(flash_mat, "emission", third_color * 0.45, BLOCK_BREAK_STEP)
	tween.set_parallel(false)
	tween.tween_interval(FLASH_IN + FLASH_HOLD)
	tween.tween_callback(func() -> void:
		if not is_instance_valid(mesh):
