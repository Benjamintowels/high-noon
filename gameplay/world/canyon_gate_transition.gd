...
## Multi-gate hotel ↔ canyon ↔ church cinematic. Letterbox + look-at camera +
## white zone title, then StageZoneCuller swaps heavy stage roots so only the
## active region draws and simulates.
...
const FALLBACK_CANYON_LOOK_LOCAL := Vector3(-22.0, 4.0, 0.0)
const FALLBACK_OUTSIDE_LOOK_LOCAL := Vector3(28.0, 4.0, 0.0)
...
class GateBinding:
	var trigger: Area3D
	var canyon_look: Marker3D
	var outside_look: Marker3D
	var outside_zone: int = Zone.OVERWORLD
	var outside_title: String = TITLE_HOTEL
	var armed := true
...
func add_gate(
	...
	outside_look_name: String = "HotelLookAt"
) -> void:
	if trigger == null:
		return
	var gate := GateBinding.new()
	gate.trigger = trigger
	gate.outside_zone = outside_zone
	gate.outside_title = outside_title
	_configure_trigger(trigger)
	_resolve_gate_look_markers(gate, outside_look_name)
...
func _resolve_gate_look_markers(gate: GateBinding, outside_look_name: String) -> void:
	gate.canyon_look = gate.trigger.get_node_or_null("CanyonLookAt") as Marker3D
	gate.outside_look = gate.trigger.get_node_or_null(outside_look_name) as Marker3D
	if gate.outside_look == null:
		# Accept either HotelLookAt or ChurchLookAt if the caller name missed.
		gate.outside_look = gate.trigger.get_node_or_null("HotelLookAt") as Marker3D
		if gate.outside_look == null:
			gate.outside_look = gate.trigger.get_node_or_null("ChurchLookAt") as Marker3D
	if gate.canyon_look == null:
		gate.canyon_look = Marker3D.new()
		gate.canyon_look.name = "CanyonLookAt"
		gate.trigger.add_child(gate.canyon_look)
		gate.canyon_look.position = FALLBACK_CANYON_LOOK_LOCAL
	if gate.outside_look == null:
		gate.outside_look = Marker3D.new()
		gate.outside_look.name = outside_look_name
		gate.trigger.add_child(gate.outside_look)
		gate.outside_look.position = FALLBACK_OUTSIDE_LOOK_LOCAL
...
func _on_body_entered(body: Node3D, gate: GateBinding) -> void:
	...
	if _active or not gate.armed:
		return
	_begin_sequence(gate, _resolve_entering_canyon(body, gate))
...
func _resolve_entering_canyon(player: Node3D, gate: GateBinding) -> bool:
	var into := _into_canyon_direction(gate)
...
func _into_canyon_direction(gate: GateBinding) -> Vector3:
	if gate.canyon_look == null or gate.outside_look == null:
		return Vector3.LEFT
	var into := gate.canyon_look.global_position - gate.outside_look.global_position
	into.y = 0.0
...
func _begin_sequence(gate: GateBinding, entering_canyon: bool) -> void:
	if _active:
		return
	_active = true
	gate.armed = false

	var dest_zone: int = Zone.CANYON if entering_canyon else gate.outside_zone
	var look_target := gate.canyon_look if entering_canyon else gate.outside_look
	var title := TITLE_CANYONS if entering_canyon else gate.outside_title
	var into := _into_canyon_direction(gate)
	var walk_dir := into if entering_canyon else -into
...
func _apply_zone_state(zone: int) -> void:
	...
	# Combatants live under stage/CanyonBandits. Only toggle visibility/process —
	# do NOT run StageZoneCuller here (it disables CollisionShape3Ds on the NPCs
	# and leaves them frozen/broken across zone toggles).
	if _stage != null:
	...
	# NewGameHotel + church/canyon gates stay outside culled roots so Area3Ds
	# keep working. Church is owned by DistanceZoneCuller in overworld mode.
...
func _detect_zone(world_pos: Vector3) -> int:
	var past_hotel_into_canyon := false
	var on_church_side := false
	for gate in _gates:
		if gate.canyon_look == null or gate.outside_look == null:
			continue
		var to_canyon := gate.canyon_look.global_position.distance_squared_to(world_pos)
		var to_outside := gate.outside_look.global_position.distance_squared_to(world_pos)
		var on_canyon_side := to_canyon < to_outside
		if gate.outside_zone == Zone.OVERWORLD:
			past_hotel_into_canyon = on_canyon_side
		elif gate.outside_zone == Zone.CHURCH:
			on_church_side = not on_canyon_side

	if on_church_side:
		return Zone.CHURCH
	if past_hotel_into_canyon:
		return Zone.CANYON
